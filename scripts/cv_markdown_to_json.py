...
def extract_author_info(config):
    ...
    author_info = {
        ...
        "summary": "",
        "location": {"address": "", "postalCode": "", "city": "", "countryCode": "US", "region": ""},
        "profiles": [],
    }
...
def parse_skills(skills_text):
    """Parse multi-level markdown skills section into structured JSON list."""
    skills_entries = []

    # 先拆分每个一级技能类别（以单个星号开头的行）
    categories = re.split(r"(?m)^\* ", skills_text)
    ...
    for block in categories:
        ...
        # 第一个非空行为类别名（去掉多余 Markdown 符号）
        category_name = re.sub(r"[\*\#\_`]", "", lines[0]).strip()

        # 之后的行如果以两个空格或两个空格+星号开头，则是子技能
        subskills = []
        for line in lines[1:]:
            line = line.strip()
            if line.startswith("*") or line.startswith("-"):
                ...
                if clean_line:
                    subskills.append(clean_line)

        # 如果没有找到子项（有时写成单行）
        if not subskills and len(lines) > 1:
            subskills = [re.sub(r"[\*\#\_`]", "", l).strip() for l in lines[1:] if l.strip()]

        # 保存结果
        skills_entries.append({"name": category_name, "level": "", "keywords": subskills})
